...
- Серверные инварианты из промпта продублированы в коде (`src/lib/prompts/style-hints.ts`): явный стиль пользователя («фотореализм», «аниме», «рентген» и т.п.) вытесняет якорь медиума, а финальная формула с медиумом гарантируется постобработкой
