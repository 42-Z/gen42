...
- Политика текста: слова и лозунги появляются в кадре только по запросу. `requestsText`/`extractQuotedTexts` определяют текстовый запрос; без него слоган-якорь не передаётся, а любые кавычки в ответе маскируются постобработкой (`maskUnrequestedTexts`); точный текст пользователя переносится дословно и проверяется (`EXACT TEXT`)
- Плотность и экшен: промпт требует три слоя кадра, минимумы по существам/технике/роскоши/архитектуре, ≥3 сюрприза и явное действие в первом предложении с усилением в 2–3 местах; реализм материалов и света обязателен, `toy-like`/`flat` запрещены
