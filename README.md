...
Ключи обоих провайдеров хранятся в таблице `api_keys` (`provider`: `huggingface` — генерация картинок, `poolside` — обогащение промпта) и управляются через админ-панель: добавление, проверка с включением, удаление. У HF-ключей видны остатки обеих квот ZeroGPU (секунды и прогоны), а «Проверить и включить» включает ключ только если есть и секунды, и прогоны. В переменных окружения ключи не хранятся, API отдаёт их в маскированном виде.
