...
- Изображения генерирует Krea-2 Space на HuggingFace через пул API-ключей: у бесплатного ZeroGPU суточные лимиты секунд GPU и прогонов (8 на токен), исчерпанные ключи пропускаются ещё до вызова и возвращаются сами после сброса; если LLM-ключи кончились, промпт собирается шаблоном — генерация не падает
- Кредиты: у каждого пользователя баланс, одна генерация = один кредит; при регистрации сразу даётся 1 стартовый кредит; админ может менять баланс
