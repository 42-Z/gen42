...
export function MeshBackground({ className }: { className?: string }) {
	return (
		<div
			className={cn(
				"pop-mesh pointer-events-none absolute inset-0 -z-10",
				className,
			)}
			aria-hidden="true"
		/>
...
export function EmptyCanvasArt({ className }: { className?: string }) {
	return (
		<svg
			...
			className={className}
			role="img"
			aria-label="Пустой холст ждёт первую картинку"
		>
			<rect
				x="52"
				y="28"
...
export function PopSpinner({ className }: { className?: string }) {
	return (
		<svg
			viewBox="0 0 48 48"
			fill="none"
			className={cn("animate-pop-spin", className)}
			aria-hidden="true"
		>
			<circle
				cx="24"
			...
			/>
			<path
				d="M24 5a19 19 0 0 1 16.5 9.5"
				stroke="url(#pop-spin-g)"
				strokeWidth="4"
				strokeLinecap="round"
			/>
			<defs>
