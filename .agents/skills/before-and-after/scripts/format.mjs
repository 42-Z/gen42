...
export function localRef(file, cwd = process.cwd()) {
	...
	if (
		...
		throw new Error(
			`Media files must be inside the working directory: "${file}"`,
		);
	}
	const ref = path.startsWith(".") ? path : `./${path}`;
	if (/\s/.test(ref))
		throw new Error(`Media paths cannot contain whitespace: "${ref}"`);
	return ref;
}

export function buildPairs({ before = [], after = [], labels = [] }) {
	if (after.length === 0)
		throw new Error("At least one --after file is required");
	if (before.length !== 0 && before.length !== after.length) {
		throw new Error(
			"Provide either no --before files or one --before file for every --after file",
		);
	}
...
function githubAttachmentUrl(value) {
	...
	if (
		...
		throw new Error(
			`Expected a https://github.com/user-attachments/assets/... URL: "${value}"`,
		);
	}
	return url.href;
}
...
function renderImagePair(pair, cwd, lines) {
	...
	if (pair.before) {
		...
		lines.push(
			`| Preview${suffix} |`,
			"|:---:|",
			`| ${imageRef(pair.after, "Preview", cwd)} |`,
			"",
		);
	}
}
...
function renderVideoPair(pair, cwd, lines) {
	...
	if (pair.before) {
		lines.push(
			`**Before${suffix}**`,
			"",
			imageRef(pair.before, "Before", cwd),
			"",
		);
	}
	lines.push(
...
function renderVideoTablePair(pair, lines) {
	...
	lines.push(
		`    <th>${beforeUrl ? "After" : "Preview"}${suffix}</th>`,
		"  </tr>",
		"  <tr>",
	);
	if (beforeUrl)
		lines.push(
			`    <td><video src="${beforeUrl}" width="100%" controls></video></td>`,
		);
	lines.push(
...
export function replaceMarkedBlock(body, block) {
	...
	if (start === -1 || end === -1 || end < start) {
		throw new Error(
			"PR body contains an incomplete before-and-after marker block",
		);
	}
	if (
...
function validateFiles(pairs, cwd) {
	for (const file of pairs
		...
		if (!existsSync(absolute))
			throw new Error(`Media file does not exist: "${file}"`);
		mediaKind(file);
		localRef(file, cwd);
	}
}
...
function main(argv) {
	...
	const localMode =
		(values.after?.length ?? 0) > 0 || (values.before?.length ?? 0) > 0;
	const videoTableMode =
		(values["after-video-url"]?.length ?? 0) > 0 ||
		(values["before-video-url"]?.length ?? 0) > 0;
	...
	if (values["attach-list"]) {
		if (videoTableMode)
			throw new Error("--attach-list requires local media files");
	...
	if (values["body-file"]) {
		process.stdout.write(
			replaceMarkedBlock(readFileSync(values["body-file"], "utf-8"), block),
		);
	} else {
...
if (
	process.argv[1] &&
	import.meta.url.endsWith(process.argv[1].split("/").at(-1))
) {
	try {
